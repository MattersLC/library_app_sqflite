...
class SolicitudesPorEstudiante extends StatefulWidget {
  final String numControl;
  final String type;
  const SolicitudesPorEstudiante(
      {super.key, required this.numControl, required this.type});

  @override
  State<SolicitudesPorEstudiante> createState() =>
      _SolicitudesPorEstudianteState();
}

class _SolicitudesPorEstudianteState extends State<SolicitudesPorEstudiante> {
  List<Solicitud> _solicitudesPorEstudiante = [];
  ...
  Future<void> _getSolicitudes() async {
    final solicitudes =
        await DatabaseHelper.getSolicitudesPorEstudiante(widget.numControl);
    if (solicitudes != null) {
      setState(() {
        _solicitudesPorEstudiante = solicitudes;
      });
    }
  }

  Future<void> _getSolicitudesActivas() async {
    final solicitudes = await DatabaseHelper.getSolicitudesPorEstudianteActivas(
        widget.numControl);
    if (solicitudes != null) {
      setState(() {
        _solicitudesPorEstudiante = solicitudes;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Prestamos de ${widget.numControl}',
            style:
      ...
      body: _solicitudesPorEstudiante.isEmpty
          ? Center(
              child: Text(
          ...
          : ListView.builder(
              itemCount: _solicitudesPorEstudiante.length,
              itemBuilder: (context, index) =>
                  solicitudesTile(_solicitudesPorEstudiante[index]),
            ),
    );
  }
